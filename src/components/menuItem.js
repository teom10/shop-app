...
export default class MenuItem extends Component {
  ...
  componentDidMount() {
    debugger;
    const { name, price , pricewithtax} = this.props;
 ...
 // function for calculate price with tax 
 handleCalculatePriceWithTax = ({price}) => {
   debugger;
  const updatedItem = {
    tax : price * 1
  } 
    ...
    return (
      <>
        ...
        <div className="col-md-6">
          <div className="row">
      <label name="tax">{this.handleCalculatePriceWithTax}</label>
          </div>
        </div>
      </>
    )
